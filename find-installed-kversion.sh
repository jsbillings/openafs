...
if grep -q RHEL_RELEASE /usr/include/linux/version.h; then
        ...
        DISTROVERSION="el$RHELMAJOR"
    if [[ "$RHELMAJOR" -ge 8 ]]; then
        RHELMINOR=$(grep RHEL_MINOR /usr/include/linux/version.h | cut -d' ' -f3)
        DISTROVERSION="${DISTROVERSION}_${RHELMINOR}"
    fi
else # assume fedora
        DISTROVERSION=fc$(cut -d' ' -f3 /etc/fedora-release)
...
printf "%%define kversion %d.%d.%d-%s.%s\n" $KVERSION1 $KVERSION2 $KVERSION3 $RVERSION $DISTROVERSION
